...
public int recursiveBinarySearch(int catNumToFind, int nLow, int  nHigh)
{
  ...
  if(store[o].getCatNum()==catNumToFind){
    return store[o].getInventory();
  }
  else if(store[o].getCatNum()<catNumToFind){
    return recursiveBinarySearch(catNumToFind,o+1, nHigh);
  }
  else{
    return recursiveBinarySearch(catNumToFind,nLow,o-1);
  }
}
public void setup()
{
